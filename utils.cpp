...
void sec_to_str(long ls, char *str)
{
    ...
    if (sec_to_td(ls, &t) != -1) {
        temp = (t.year > 2000)? t.year - 2000: t.year - 1900;

	sprintf(str, "%02d.%02d.%02d %02d:%02d:%02d", t.day, t.mon, temp, t.hour, t.min, t.sec);
    } else {
...
//---------------------------------------------------------------------------
void ver_to_str(DEV_ADDR_STRUCT * addr, char *str)
{
    TIME_DATE t;

    //ver: 12.06.13 11.24

    if (sec_to_td(addr->time, &t) != -1) {
